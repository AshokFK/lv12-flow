<div>
    ...
    <div class="relative mb-4 w-full">
        <flux:heading size="xl" level="1">Proses</flux:heading>
        <flux:subheading size="lg" class="mb-6">List master data Proses</flux:subheading>
    ...
    <div class="flex flex-col">
                ...
                <div class="overflow-hidden shadow">
                    <table class="min-w-full divide-y divide-gray-200 table-fixed">
                        <thead>
                            <tr class="border-b border-t border-gray-200">
                                <th scope="col" class="p-2 text-accent">
                                    <button type="button" class="flex items-center justify-between w-full cursor-pointer text-sm font-semibold text-left uppercase" wire:click="sortBy('mastercode')">
                                        Mastercode
                                        @if ($sortColumn === 'mastercode')
                                        <span class="ml-1">
                                ...
                                <th scope="col" class="p-2 text-accent">
                                    <button type="button" class="flex items-center justify-between w-full cursor-pointer text-sm font-semibold text-left uppercase" wire:click="sortBy('nama_jp')">
                                        Nama JP
                                        @if ($sortColumn === 'nama_jp')
                                        <span class="ml-1">
                                            @if($sortDirection === 'asc' )
                                            <flux:icon.chevron-up variant="micro" />
                                            @else
                                ...
                                <th scope="col" class="p-2 text-accent">
                                    <button type="button" class="flex items-center justify-between w-full cursor-pointer text-sm font-semibold text-left uppercase" wire:click="sortBy('level')">
                                        Level
                                        @if ($sortColumn === 'level')
                                        <span class="ml-1">
                                            @if($sortDirection === 'asc' )
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @forelse ($this->listProses as $proses)
                            <tr>
                            ...
                            @empty
                            <tr class="border-b">
                                <td class="p-2 text-base font-medium text-center text-accent" colspan="7">Tidak ada data
        ...
        <div class="sticky bottom-0 right-0 items-center w-full py-4 bg-white border-t border-gray-200 dark:bg-accent-foreground">
            {{ $this->listProses->links() }}
        </div>
    </div>

    {{-- modal create proses --}}
    <livewire:proses.create-proses />

    {{-- modal edit proses --}}
    <livewire:proses.edit-proses />

    {{-- modal delete proses --}}
    <flux:modal name="delete-proses" class="min-w-[22rem]">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">Delete proses?</flux:heading>
                <flux:text class="mt-2">
                    <p>Anda yakin akan menghapus proses ini ?</p>
                </flux:text>
            </div>
            <div class="flex gap-2">
                ...
                <flux:button wire:click="delete" variant="danger">Delete proses</flux:button>
            </div>
        </div>
    </flux:modal>
